...
def create_with_remote_venv(python_exe, penv_dir):
    ...
    venv_script_path = Path(dir_path).joinpath("virtualenv.pyz")
...
def install_pip(python_exe, penv_dir):
    ...
    try:
        ...
        print(Path(dir_path).joinpath("../../../.rt_global").absolute().as_posix())
        if Path(dir_path).joinpath("../../../.rt_global").exists():
            click.echo("global")
        ...
        get_pip_path = Path(dir_path).joinpath("get-pip.py")
        click.echo(str(get_pip_path))
